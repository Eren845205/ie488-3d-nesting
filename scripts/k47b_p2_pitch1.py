# -*- coding: utf-8 -*-
"""k47b_p2_pitch1.py — K-47b: plan2 NFV @pitch=1.0 (H-17 dilimli-FFT kapisi).

Gerekce (2026-07-12): K-39b'de p1.0 bacagi MemErr'le atlanmisti; H-17 acti.
plan2 @2.0: ham 532.0 INVALID(29 kilit) / guard 544.5 CIFT-LEGAL SAMPIYON.
@1.0 hipotezi: kuantizasyon vergisi geri gelir -> 532-alti legal potansiyel.
Recete K-41/44: ONCE ham; (b+c) kilit>0 CIKARSA guard bacagi. @2.0'da ham
29 kilitti -> guard bacagi muhtemel; kosullu zincir scriptte.
RAM/GPU zinciri: k47a_d5_pitch1.log son satiri BITTI olana kadar bekler.
REF (sampiyon @2.0): 544.5 guard. MANUEL 492.39.
Kosum: python -m scripts.detach_run k47b_p2_pitch1    SAF ASCII.
"""
...
LOG = Path(__file__).parent / "k47b_p2_pitch1.log"
BEKLE = Path(__file__).parent / "k47a_d5_pitch1.log"
PLATE = (335.0, 335.0)
NOGO = ((152.5, 0.2), (185.5, 45.0))
PITCH = 1.0
REF = 544.5     # p2 sampiyonu (K-41 guard @2.0)
MANUEL = 492.39
...
def _bacak(inst, n_total, etiket, guard):
    ...
    if legal_bc is not None and hv < REF:
        out = _ROOT / "results" / f"plan2_p1_{etiket}_{legal_bc:.1f}mm.stl"
...
def main():
    ...
    log("K-47b PLAN2 NFV @1.0 — H-17 dilimli-FFT kapisi (ref 544.5 / manuel 492.39)")
    tur = 0
    while True:
        ...
        if satirlar and satirlar[-1].endswith("BITTI"):
            break
        if tur % 10 == 0:
            log(f"k47a bekleniyor ({time.strftime('%H:%M')})")
        tur += 1
        time.sleep(300)
    log("k47a bitti — plan2 @1.0 basliyor")

    inst = _load_instance("plan2")
    n_total = sum(int(p.qty) for p in inst.parts)
    log(f"plan2: {n_total} parca hedef  pitch={PITCH}")
