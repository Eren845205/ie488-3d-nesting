# -*- coding: utf-8 -*-
"""k39b_plan2_nfv_v2.py — K-39b: plan2 NFV v2 (guvenli pitch merdiveni).

K-39 v1 OLDU: fine_pitch=None → suggest_nfv_pitch ~0.69 secti (7.2mm kanatlar)
→ (486,432,625) float64 = 1001MiB MemoryError. BULGU: suggest_nfv_pitch bellek
guard'i dense float64 ara-array'i hesaba katmiyor (gunduz isi — ayri fix).
K-38 dersi: 2mm kuralinda efektif bosluk = ceil(2/pitch)*pitch → TAM olan
pitch'ler: 2.0 (1vox) ve 1.0 (2vox). (1,2) araligi zehirli (3.5mm sisme).
Merdiven: (A) pitch=2.0 (guvenli, ~10M hucre) → (B) pitch=1.0 (~79M hucre,
RAM riskli — MemoryError yakalanir, A sonucu korunur).
RAM zinciri: k40_plan1_nfv.log son satiri BITTI olana kadar bekler.
Kosum: python -m scripts.detach_run k39b_plan2_nfv_v2    SAF ASCII.
"""
from __future__ import annotations
import sys, time
from pathlib import Path
_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(_ROOT))

import trimesh
from src.nesting3d.accessibility import check_separability_5dir
from src.nesting3d.rotation_extract import check_separability_rot
from src.nesting3d.clearance import min_clearance
from src.nesting3d.coarse_to_fine import clearance_to_voxels
from src.nesting3d.export_stl import placed_meshes
from src.nesting3d.nfv_solve import solve_nfv
from scripts.eval_gate import _load_instance, legal_of

LOG = Path(__file__).parent / "k39b_plan2_nfv_v2.log"
BEKLE = Path(__file__).parent / "k40_plan1_nfv.log"
PLATE = (335.0, 335.0)
NOGO = ((152.5, 0.2), (185.5, 45.0))
REF_HEIGHTMAP = 618.0
PITCH_MERDIVENI = (2.0, 1.0)


def log(m=""):
    print(m, flush=True)
    with LOG.open("a", encoding="utf-8") as fh:
        fh.write(m + "\n")


def _bacak(inst, n_total, pitch):
    etiket = f"p{pitch:g}"
    t = time.perf_counter()
    try:
        r = solve_nfv(inst, plate_w_mm=PLATE[0], plate_d_mm=PLATE[1],
                      quality="max", seed=42, clearance_mm=2.0,
                      no_go_bounds=NOGO, fine_pitch=pitch)
    except MemoryError as e:
        log(f"[{etiket}] MemoryError — bacak atlandi ({e})")
        return None
    except Exception as e:
        log(f"[{etiket}] EXCEPTION: {type(e).__name__}: {e}")
        return None
    px = float(r.fine_pitch)
    parts = r.fine_voxel_parts
    pls = list(r.placements)
    hv = max((p.z + parts[p.part_id].orientations[p.orientation_idx]
              .grid.shape[2]) for p in pls) * px
    log(f"[{etiket}] NFV: h={hv:.1f}  pitch={px}  yerlesen={len(pls)}/{n_total}"
        f"  ({(time.perf_counter() - t) / 60:.1f} dk)")
    r5 = check_separability_5dir(pls, parts)
    rot = check_separability_rot(pls, parts, max_grid_vox=800,
                                 sure_butcesi_s=480.0,
                                 erode_clearance_vox=clearance_to_voxels(2.0, px))
    log(f"[{etiket}] (b) kilit={r5.n_locked}  (b+c) kilit={rot.n_locked}  "
        f"cert={len(rot.certificates)}")
    meshes = placed_meshes(pls, parts, px)
    rep = min_clearance(meshes)
    legal_b, _ = legal_of(hv, len(pls), n_total, float(rep.min_mm),
                          int(r5.n_locked), clearance_req=2.0)
    legal_bc, reason = legal_of(hv, len(pls), n_total, float(rep.min_mm),
                                int(rot.n_locked), clearance_req=2.0)
    log(f"[{etiket}] SONUC: h={hv:.1f}  clear={rep.min_mm:.3f}  legal(b)="
        f"{legal_b if legal_b is not None else 'INVALID'}  legal(b+c)="
        f"{legal_bc if legal_bc is not None else 'INVALID(' + str(reason) + ')'}")
    if legal_bc is not None and hv < REF_HEIGHTMAP:
        out = _ROOT / "results" / f"plan2_nfv_{etiket}_{legal_bc:.1f}mm.stl"
        trimesh.util.concatenate(meshes).export(out)
        log(f"[{etiket}] STL: {out}")
    return (hv, legal_bc)


def main():
    LOG.write_text("", encoding="utf-8")
    log("K-39b PLAN2 NFV v2 — pitch merdiveni 2.0/1.0 (heightmap ref 618.0 / manuel 492.39)")
    tur = 0
    while True:
        satirlar = ([s.strip() for s in
                     BEKLE.read_text(encoding="utf-8", errors="ignore").splitlines()
                     if s.strip()] if BEKLE.exists() else [])
        if satirlar and satirlar[-1].endswith("BITTI"):
            break
        if tur % 10 == 0:
            log(f"k40 bekleniyor ({time.strftime('%H:%M')})")
        tur += 1
        time.sleep(300)
    log("k40 bitti — plan2 NFV v2 basliyor")

    inst = _load_instance("plan2")
    n_total = sum(int(p.qty) for p in inst.parts)
    log(f"plan2: {n_total} parca hedef")
    sonuclar = {}
    for pitch in PITCH_MERDIVENI:
        s = _bacak(inst, n_total, pitch)
        if s is not None:
            sonuclar[pitch] = s
    if sonuclar:
        en_iyi = min((hv, p) for p, (hv, lg) in sonuclar.items())
        log(f"HUKUM-VERISI: " + " | ".join(
            f"p{p:g}={hv:.1f}" for p, (hv, lg) in sorted(sonuclar.items()))
            + f" | heightmap 618.0 | manuel 492.39 -> "
            f"{'REKOR' if en_iyi[0] < REF_HEIGHTMAP else 'ref gecilemedi'}")
    else:
        log("HUKUM-VERISI: hicbir bacak tamamlanamadi")
    log("BITTI")


if __name__ == "__main__":
    main()
