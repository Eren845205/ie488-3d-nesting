...
def main():
    ...
    log("PLAN1 URETIM (dblf_only x5 seed) @335+NOGO — Magics 110.41 / eski-plaka default 125.0")
    cfg = DATASETS["plan1"]
    ...
    kw["menu"] = {"dblf_only": build_menu()["dblf_only"]}  # tam-portfoy@1.0 = 8h+ (olculdu) -> dblf_only
    best = None
    for sd in (42, 13, 7, 5, 21):
        kw["seed"] = sd
        ts = time.perf_counter()
        try:
            rr = solve_coarse_to_fine(inst, plate_w_mm=PLATE[0], plate_d_mm=PLATE[1], **kw)
        except Exception as e:
            log(f"[s{sd}] EXCEPTION: {e}")
            continue
        log(f"[s{sd}] h={float(rr.height_mm):.1f}  ({(time.perf_counter()-ts)/60:.1f} dk)")
        if best is None or float(rr.height_mm) < float(best.height_mm):
            best = rr
    if best is None:
        log("EXCEPTION: hicbir seed cozum vermedi")
        log("BITTI")
        return
    r = best
    ...
    if legal is not None:
        out = _ROOT / "results" / f"plan1_uretim335_{legal:.1f}mm.stl"
