# -*- coding: utf-8 -*-
"""k41_plan2_exit_guard.py — K-41: plan2 NFV @2.0 + exit_guard (legallesme denemesi).

K-39b p2.0 kaniti: plan2 NFV ham = 532.0, clear 2.002, AMA (b) 61 / (b+c) 29
kilit → INVALID. plan3 emsali (K-30v2/K-33): exit_guard EXACT ham 626'yi
+66 vergiyle 692'ye tasiyip 5-yon kilit 0 yapti; coklu-aday 680'e indirdi.
Ayni recete: 532 + vergi ~50-90 → 580-620 bandi; ref 618.0 ALTI mumkun.
K-29 dersi: tamir degil ONLEME (repair_separability plan3'te felaketti).
Tek bacak: pitch=2.0 + exit_guard=True (retries default 2; K-33b: 2→6 fark yok).
RAM zinciri: k39b_plan2_nfv_v2.log son satiri BITTI olana kadar bekler.
Kosum: python -m scripts.detach_run k41_plan2_exit_guard    SAF ASCII.
"""
...
LOG = Path(__file__).parent / "k41_plan2_exit_guard.log"
BEKLE = Path(__file__).parent / "k39b_plan2_nfv_v2.log"
PLATE = (335.0, 335.0)
...
def main():
    ...
    log("K-41 PLAN2 EXIT-GUARD — NFV @2.0 + guard (ham 532.0 / heightmap 618.0 / manuel 492.39)")
    tur = 0
    while True:
        ...
        if satirlar and satirlar[-1].endswith("BITTI"):
            break
        if tur % 10 == 0:
            log(f"k39b bekleniyor ({time.strftime('%H:%M')})")
        tur += 1
        time.sleep(300)
    log("k39b bitti — exit_guard bacagi basliyor")
    ...
    log(f"NFV+guard: h={hv:.1f}  pitch={px}  yerlesen={len(pls)}/{n_total}"
    ...
    log(f"HUKUM-VERISI: guard={hv:.1f} (vergi {hv - REF_HAM:+.1f}) | heightmap 618.0 "
        f"| manuel 492.39 -> "
        f"{'REKOR' if (legal_bc is not None and hv < REF_HEIGHTMAP) else 'ref gecilemedi'}")
    if legal_bc is not None and hv < REF_HEIGHTMAP:
        out = _ROOT / "results" / f"plan2_nfv_guard_{legal_bc:.1f}mm.stl"
