# -*- coding: utf-8 -*-
"""k49a_p3_r11.py â€” K-49a: plan3 + R11 surekli z-kompaksiyon (MANUEL-GECME probu).

Gerekce (2026-07-13, Eren: "plan3'te de gecebilir miyiz?"): p3 598.5 vs manuel
593 = +5.5mm â€” en dar acik. d5'te R11 mekanizmasi 7.76mm buldu (K-48 v1;
...
def main():
    ...
    log("K-49a PLAN3 + R11 â€” manuel-gecme probu (ref 598.5 / manuel 593)")
    tur = 0
    while True:
        ...
        tur += 1
        time.sleep(300)
    log("k48b bitti â€” K-49c basliyor")
    ...
    t = time.perf_counter()
    res = continuous_z_settle(meshes, clearance_mm=2.0, pay_mm=1.2,
    ...
    try:
        ...
        log(f"kilit re-check MemoryError ({e}) — kilit olculemedi (serh)")

    h_final = res.height_mm
    legal = (rep.min_mm >= 2.0 and kilit_pre is not None
    ...
    if legal and h_final < h0:
        out = _ROOT / "results" / f"plan3_r11c_{h_final:.1f}mm.stl"
        ...
        kaydet("plan3", "nfv", "derin@2.0+r11", h_final, len(pls), n_total,
               ...
               kosu_id="K-49c/r11pay", log_yolu="scripts/k49c_p3_r11_pay.log",
