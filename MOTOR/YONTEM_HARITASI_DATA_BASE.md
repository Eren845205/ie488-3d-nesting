...
#### [K-24/Adım-1] Zincir-ekimi ÖN-ANALİZİ — gerçek plakada kapasite ölçümü
- **Durum:** 🔶 AÇIK (Adım-1 ölçüldü 2026-07-06; hüküm Adım-2 prototipinde) · **Kanıt:** `scripts/k24_on_analiz.py` + `scripts/k24_on_analiz2.py` (+logları); yerleşimler `data/mail_stl/k24_placements_325_264.pkl` (izlenmiyor, 59MB — sonraki adımlar re-solve etmesin)
...
- **Adım-2 tanımı (sıradaki):** decode prototipi — ASY bloğunda halka→zincir dengeli routing (her ASY atanmış kök-site penceresine kısıtlı drop), kalan 524 parça üretim decode'uyla devam, tam replay `legal_height` E2E (clearance+0 kilit). Üretime dokunmadan `scripts/` içinde; beklenen maliyet ~1 koşu (~6dk, pickle+cache ile).
...
| **K-24** | Bilinçli zincir-ekimi dekodu (özdeş kabuk çanlarını giriş-ofsetiyle KASITLI tohumla) | 6GB | Yüksek | **ORTA (yükseldi 2026-07-06)** | **GERÇEK PLAKA (335→325) K-24'ü GÜÇLENDİRDİ:** height-driver teşhisi (K-25, `c3_height_driver_deneme4`) 264 tavanının TAMAMEN 62 çandan geldiğini + düğmelerin 236'da tavanladığını gösterdi → çanları 236 altına yayabilirse tavan 264→~236 = **Magics 250 ALTI**. Büyük plaka bu yayma imkânını açtı (301.6'da yer yoktu). Ölç-önce: ~~Adım-1~~ **TAMAM 2026-07-06** (§3.1 K-24/Adım-1): 13 kök / dengesiz 36-kule / s=11.2; post-hoc kapasite 0 (K-22 teyidi) ama SIRA-İÇİ dengeli dağıtım analitik ~236 → ödül ~28mm Magics-altı. **Sıradaki = Adım-2 prototip** (dengeli routing replay, `scripts/` only). |
