...
| M19 | **Not→YÖNLENDİRME derleyicisi** (Eren 2026-08-20: "agent algoritmayı gelen nota göre yönlendirebilmeli"): not→kısıt hattının üst katmanı — LLM çıktısı 3 tipli şema-nesnesi: (1) kısıtlar (mevcut), (2) MEKANİZMA-UYGUNLUK filtreleri (duruş-koru notu → çapraz/tilt kolları o parçalarda aday-dışı, kafes duruş-koru moduna; belirsiz yorum → onay ekranında seçenek, otomatikte konservatif), (3) POLİTİKA sinyalleri (bölme/tek-plaka/öncelik → U1). Güvenlik aynen: hakem + gölge + onay kapısı; LLM motor parametresi YAZMAZ. Yan ürün: çift-yol ile "notunuzun bedeli X mm" müşteri-rapor satırı | K-56g hattı + M14 | kablo |
